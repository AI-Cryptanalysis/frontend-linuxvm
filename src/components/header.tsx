...
export function Header({ onMenuClick }: HeaderProps) {
  ...
  const handleLogout = () => {
    // Mock logout logic: Clear local state/cookies if needed
    router.push("/login");
  };
    ...
    <header className="h-20 bg-surface/60 backdrop-blur-xl border-b border-border/5 flex items-center justify-between px-8 sticky top-0 z-40">
      ...
      <div className="flex items-center gap-6">
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="icon" className="w-10 h-10 rounded-xl relative hover:bg-surface-container-high" aria-label="Notifications">
            <Bell className="w-5 h-5 text-muted-foreground" />
            <span className="absolute top-2.5 right-2.5 w-2 h-2 bg-red-500 rounded-full border-2 border-surface" />
          </Button>

          <Button variant="ghost" size="icon" className="w-10 h-10 rounded-xl hover:bg-surface-container-high hidden sm:flex" aria-label="Fullscreen">
          ...
          <div className="flex items-center gap-3 pl-2">
            <div className="flex flex-col items-end justify-center hidden sm:flex">
              <span className="text-sm font-sans font-bold text-foreground">Admin_Nova</span>
              <SecurityPulse label="SECURE" className="h-5 px-2 bg-primary/5 border-none" />
            </div>
            <Avatar className="w-10 h-10 rounded-2xl border-2 border-surface-container-high galactic-shadow hover:scale-105 transition-transform duration-300">
